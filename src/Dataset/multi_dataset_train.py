...
class Qformer_multi_dataset_train(Dataset):
    def __init__(self, text_tokenizer, max_seq=2048, max_img_size=100, image_num=32, voc_size=32000):
        ...
        # CTRG
        ctrg_dataset = CTRG_Impression_Finding_Dataset(
            data_dir='/data/chenzhixuan/data/CTRG/',
            csv_path='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_data_csv/CTRG_train.csv',
            prompt_json_file='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_dataset/report_prompt.json', 
            down_sample_ratio=1)
        self.dataset_reflect['ctrg_dataset'] = ctrg_dataset
        self.data_whole_3D = self.data_whole_3D + \
            [{'ctrg_dataset': i} for i in range(len(ctrg_dataset))]
    ...
    def __getitem__(self, idx):
        ...
        '''
        Dict:{
             "image_dict": [{"image": image, "position": {"question1": 0}}], # image is a tensor of shape [c,w,h,d] like, [3,512,512,4], position is a dict, random choice of 0 or len(question)
        ...
        '''
        images = sample["image_dict"]
        position = [img['position'] for img in images]
        question1 = sample["question1"]
        question2 = sample["question2"]
        impression = sample["impression"]
        finding = sample["finding"]
        # NOTE: the following function will add image tokens to the question and answer
        images, question1, impression = self.text_add_image(
            images, question1, impression)
        ...
        self.text_tokenizer.padding_side = "right"
        text_tensor = self.text_tokenizer(
            question1 + ' ' + impression, max_length=self.max_seq, truncation=True, padding="max_length", return_tensors="pt"
        )
        lang_x = text_tensor["input_ids"][0]
        attention_mask = text_tensor["attention_mask"][0]
        try:
        ...
        if emphasize_words != []:
            ...
            key_embeddings = []
        question_tensor = self.text_tokenizer(
            question1, max_length=self.max_seq, truncation=True, padding="max_length", return_tensors="pt"
        )
        question_length = torch.sum(question_tensor["attention_mask"][0])
        labels = lang_x.clone()
        labels[labels == self.text_tokenizer.pad_token_id] = -100
        ...
        'question2': question2,'impression': impression, 'finding': finding, 'position': position,
        'loss_reweight': reweight_tensor, 'key_words_query': emphasize_words}
    ...
    def text_add_image(self, images, question, answer):
        ...
        for index, image in enumerate(images):
            ref_image.append(image["image"])
            position = image["position"]
            position = list(position.items())[0] # the position of image in the first input
            if position[0] == 'question1':
                insert_loc = position[1] - 1
                if insert_loc < 0:
                    insert_loc = 0
                question_list[insert_loc].append(index)
            if position[0] == 'answer1':
                insert_loc = position[1] - 1
                if insert_loc < 0:
                    insert_loc = 0
                answer_list[insert_loc].append(index)
...
class Qformer_multi_dataset_train_stage1(Dataset):
    def __init__(self, text_tokenizer, max_seq=2048, max_img_size=100, image_num=32, voc_size=32000):
        ...
        # CTRG
        ctrg_dataset = CTRG_Impression_Dataset(
            data_dir='/data/chenzhixuan/data/CTRG/',
            csv_path='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_data_csv/CTRG_train.csv',
            prompt_json_file='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_dataset/report_prompt.json', 
            down_sample_ratio=1)
...
class Qformer_multi_dataset_train_stage2(Dataset):
    def __init__(self, text_tokenizer, max_seq=2048, max_img_size=100, image_num=32, voc_size=32000):
        ...
        # CTRG
        ctrg_dataset = CTRG_Finding_Dataset(
            data_dir='/data/chenzhixuan/data/CTRG/',
            csv_path='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_data_csv/CTRG_train.csv',
            prompt_json_file='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_dataset/report_prompt.json', 
            impression_csv = '/home/chenzhixuan/Workspace/R2Gen/results/CTRG_impression_slice30_R2GenModel_plus_v2_2/train_res.csv',
            down_sample_ratio=1)
        self.dataset_reflect['ctrg_dataset'] = ctrg_dataset
        self.data_whole_3D = self.data_whole_3D + \
            [{'ctrg_dataset': i} for i in range(len(ctrg_dataset))]
    ...
    def __getitem__(self, idx):
        ...
        impression_list = [" ".join(s.split()) for s in impression_list if s]

        # NOTE: the following function will add image tokens to the question and answer
        images, question2 = self.text_add_image2(
            images, question2, impression_list)
        ...
        self.text_tokenizer.padding_side = "right"
        text_tensor = self.text_tokenizer(
            question2 + ' ' + finding, max_length=self.max_seq, truncation=True, padding="max_length", return_tensors="pt"
        )
        lang_x = text_tensor["input_ids"][0]
        attention_mask = text_tensor["attention_mask"][0]
        try:
        ...
        question_tensor = self.text_tokenizer(
            question2, max_length=self.max_seq, truncation=True, padding="max_length", return_tensors="pt"
        )
        question_length = torch.sum(question_tensor["attention_mask"][0])
        labels = lang_x.clone()
        labels[labels == self.text_tokenizer.pad_token_id] = -100
        ...
        if dataset_index == 'paper_inline_dataset':
            emphasize_words = []
        # print(labels,key_embeddings,reweight_tensor)
        return {'vision_x': vision_x, 'lang_x': lang_x, 'attention_mask': attention_mask, 'labels': labels, 'impression': impression_list,
        'loss_reweight': reweight_tensor, 'key_words_query': emphasize_words}

    def text_add_image2(self, images, question2, impression_list):
        ref_image = []
        ...
        prefix = ''
        if insert_loc==0:
            for imp_i in range(len(impression_list)):
                prefix = prefix + '<image>' + \
                            self.image_padding_tokens[imp_i+1] + '</image>' + \
                            impression_list[imp_i] + '. '
        else:
            for imp_i in range(len(impression_list)):
                prefix = prefix + impression_list[imp_i] + '<image>' + \
                            self.image_padding_tokens[imp_i+1] + '</image>' + '. '

        new_question = prefix + new_question

        return ref_image, new_question

class MQ_Qformer_train_dataset(Dataset):
    def __init__(self, text_tokenizer, max_seq=2048, max_img_size=100, image_num=32, voc_size=32000):
        ...
        # CTRG
        ctrg_dataset = CTRG_All_Dataset(
            data_dir='/data/chenzhixuan/data/CTRG/Chest_new_1_volume/',
            data_json='/jhcnas1/chenzhixuan/CTRG/Chest_new_1/annotation2_new_labeled_abnormal.json',
            prompt_json_file='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_dataset/report_prompt.json', 
            down_sample_ratio=1)
        self.dataset_reflect['ctrg_dataset'] = ctrg_dataset
        self.data_whole_3D = self.data_whole_3D + \
    ...
    def __getitem__(self, idx):
        ...
        question2 = sample["question2"]
        impression = sample["impression"]
        label_impression = sample["label_impression"]
        finding = sample["finding"]
        # NOTE: the following function will add image tokens to the question and answer
        images, question1, impression = self.text_add_image(
            images, question1, impression)
        ...
        if dataset_index == 'paper_inline_dataset':
            emphasize_words = []
        # print(labels,key_embeddings,reweight_tensor)
        return {'vision_x': vision_x, 'lang_x': lang_x, 'attention_mask': attention_mask, 'labels': labels, 'question1': question1,
        'question2': question2,'impression': impression, 'label_impression': label_impression,'finding': finding, 'position': position,
        'loss_reweight': reweight_tensor, 'key_words_query': emphasize_words}
