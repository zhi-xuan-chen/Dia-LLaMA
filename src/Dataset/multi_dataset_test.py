...
class Qformer_multi_dataset_test(Dataset):
    def __init__(self, text_tokenizer, max_seq=2048, max_img_size=10, image_num=32, voc_size=32000):
        ...
        ctrg_dataset = CTRG_Impression_Finding_Dataset(
            ...
            csv_path='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_data_csv/CTRG_test.csv',
            prompt_json_file='/home/chenzhixuan/Workspace/LLM4CTRG/src/Dataset/my_dataset/report_prompt.json', down_sample_ratio=1)
        self.dataset_reflect['ctrg_dataset'] = ctrg_dataset
        self.data_whole_3D = self.data_whole_3D + \
            [{'ctrg_dataset': i} for i in range(len(ctrg_dataset))]
    ...
    def __getitem__(self, idx):
        ...
        # print(sample)
        belong_to = sample[0]
        sample, image_id = self.dataset_reflect[sample[0]][sample[1]]
        ...
        vision_x, question1 = self.text_add_image(
            images, question1)
        question2, position = self.text_add_image2(
            images, question2)
        
        # print(question,answer)
        # make vision_x
        try:
            vision_x = stack_images(vision_x)
        except:
            print(self.data_whole[idx].items())
            input()
        # print(vision_x.shape,question,answer)

        return {'id': image_id, 'vision_x': vision_x, 'question1': question1, 'impression': impression, 'question2': question2, 'finding': finding, 'position': position,'belong_to': belong_to}
    ...
    def text_add_image2(self, images, question):
        question_list = [[] for _ in range(len(str(question)))]

        for index, image in enumerate(images):
            position = image["position"]
            position = list(position.items())[1]
        ...
        return new_question, insert_loc
    
class MQ_Qformer_test_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'id': image_id, 'vision_x': vision_x, 'lang_x': lang_x, 'attention_mask': attention_mask, 'labels': labels, 'question1': question1,
