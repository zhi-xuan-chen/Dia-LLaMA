...
class My_DistributedBatchSampler(Sampler):
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, batch_size_2D=4, batch_size_3D=1, drop_last=False, shuffle=True):
        ...
        indices = batch_generation(
            self.dataset, self.batch_size_2D, self.batch_size_3D, self.drop_last, self.shuffle)
        ...
        if self.drop_last and len(indices) % self.num_replicas != 0:
            ...
            self.num_samples = math.ceil(
                len(indices) / self.num_replicas)  # type: ignore[arg-type]
